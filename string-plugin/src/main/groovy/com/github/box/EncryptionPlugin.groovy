...
class EncryptionPlugin extends Transform implements Plugin<Project> {
     ...
     * 处理文件目录下的class文件
     */
    private static void handleDirectoryInput(DirectoryInput directoryInput, TransformOutputProvider outputProvider, PluginConfig config) {
        //是否是目录
     ...
     * 处理Jar中的class文件
     */
    private static void handleJarInputs(JarInput jarInput, TransformOutputProvider outputProvider, PluginConfig config) {
        if (jarInput.file.getAbsolutePath().endsWith(".jar")) {
            // 创建新的jar文件
            File jar = new File(jarInput.file.getParent() + File.separator + "classes_posture" + System.currentTimeMillis() + ".jar")
            //避免上次的缓存被重复插入
            if (jar.exists()) {
                jar.delete()
            }
